...
https://github.com/cmlarsen/scad-parametric-timing-belt-generator/tree/main
...
// How the belt is arranged on the print surface
print_layout = "loop"; // [straight, loop, loop_inner, loop_outer, loop_match, loop_offset, spiral]

/* [Tooth Profile] */

// Belt tooth profile
tooth_profile = "GT2_2mm"; // [MXL, T2.5, T5, T10, GT2_2mm, GT2_3mm, GT2_5mm, AT5, HTD_3mm, HTD_5mm, HTD_8mm, 40DP, XL, L]

/* [Belt Dimensions] */

// Number of teeth (overrides Belt Length if non-zero)
tooth_count = 35; // [0:1:500]

// Belt length in mm — used only when Tooth Count is 0 (rounded up to next tooth pitch)
belt_length = 0; // [0:1:2000]

// Belt width in mm — set to 0 to use profile default
belt_width_override = 0; // [0:0.5:50]

// Backing thickness in mm — set to 0 to use profile default
backing_thickness_override = 0; // [0:0.1:10]

/* [Advanced] */

// Maximum diameter for loops and spirals in mm — set to shorter of your X or Y axis
maximum_diameter = 200; // [50:5:500]

// Rendering quality — lower = faster preview, higher = smoother curves
render_quality = 2; // [1:Fine (slow), 2:Standard, 4:Draft (fast), 8:Very Draft]

/* [Hidden] */
// (Parameters below are not shown in the Customizer — internal use only)

$fa = render_quality;

// Resolve overrides: 0 means "use profile default"
_belt_width     = belt_width_override     > 0 ? belt_width_override     : undef;
_back_thickness = backing_thickness_override > 0 ? backing_thickness_override : undef;
_tooth_count    = tooth_count > 0 ? tooth_count : undef;
_belt_length    = belt_length > 0 ? belt_length : undef;

// Tooth profile defaults ordered: [name, tooth_pitch, back_thickness, belt_width]
...
// By default this will render as a 3D object.
// Uncomment the next line to render as 2D for DXF export.
//projection()
belting(
    print_layout      = print_layout,
    tooth_profile     = tooth_profile,
    tooth_count       = _tooth_count,
    belt_length       = _belt_length,
    belting_width     = _belt_width,
    backing_thickness = _back_thickness,
    max_diameter      = maximum_diameter
);

// ─── Pitch Length Calculator ─────────────────────────────────────────────────
// Pitch length = the length measured along the neutral axis of the belt
// (i.e. at the cord/pitch line, not the tooth tips or backing surface).
// For a loop: PL = tooth_count × tooth_pitch
// For a straight: same formula, but represents total developed length.

_defaults  = tooth_profile_defaults[search([tooth_profile], tooth_profile_defaults)[0]];
_pitch     = _defaults[1];
_pl        = (_tooth_count != undef ? _tooth_count : ceil(_belt_length / _pitch)) * _pitch;
    ...
    max_diameter      = maximum_diameter)
{
    ...
    if (belt_defaults == undef) {
        ...
        echo("ERROR: Invalid belt_length and/or tooth_count in belting module.");
        ...
        echo(str("Generating a ", print_layout, " of ", tooth_profile, " belt with ", tooth_cnt,
            " teeth, ", tooth_cnt * tooth_pitch, "mm long, ", belt_width, "mm wide and ",
            back_thickness, "mm thick."));

        if (print_layout == "straight") {
            ...
            loop_belt(tooth_cnt, tooth_pitch, back_thickness, belt_width,
                      max_diameter - (back_thickness * 2), print_layout)
                belt_tooth(tooth_profile, belt_width);

        } else if (print_layout == "spiral") {
            spiral_belt(tooth_cnt, tooth_pitch, back_thickness, belt_width,
                        max_diameter - (back_thickness * 2))
                belt_tooth(tooth_profile, belt_width);

        } else {
            echo("ERROR: Invalid print_layout. Valid values: straight, loop, loop_inner, loop_outer, loop_match, loop_offset, spiral.");
        }
    }
}
...
module straight_belt(tooth_cnt, tooth_pitch, back_thickness, belt_width, max_diameter)
{
    if (tooth_pitch * tooth_cnt > max_diameter) {
        echo(str("WARNING: Straight belt is ", tooth_pitch * tooth_cnt,
            "mm long. If not printable, try spiral."));
    }
    union() {
        translate([-tooth_pitch / 2, -back_thickness, 0])
            cube([tooth_pitch * tooth_cnt, back_thickness, belt_width]);
        for (i = [0:tooth_cnt - 1]) {
...
module spiral_belt(tooth_cnt, tooth_pitch, back_thickness, belt_width, max_diameter, rot_angle = 0)
{
    ...
    // Compute radii for the current and next tooth positions.
    // Protect against negative values under the square-root which can occur
    // when parameters (max_diameter, tooth_cnt, tooth_pitch) are incompatible.
    radicand = pow(max_radius, 2) - (tooth_cnt * tooth_pitch * 2);
    if (radicand < 0) {
        echo(str("WARNING: spiral_belt radicand negative (tooth_cnt:", tooth_cnt,
                 ", tooth_pitch:", tooth_pitch, ", max_diameter:", max_diameter,") - clamping to 0."));
        radicand = 0;
    }
    radius = sqrt(radicand);

    next_radicand = pow(max_radius, 2) - ((tooth_cnt - 1) * tooth_pitch * 2);
    if (next_radicand < 0) {
        next_radicand = 0;
    }
    next_radius = sqrt(next_radicand);

    rad_diff = next_radius - radius;
    // Avoid divide-by-zero when radius is zero.
    angle = radius == 0 ? 0 : atan(tooth_pitch / radius);

    if (tooth_cnt > 0) {
        union() {
            spiral_belt((tooth_cnt - 1), tooth_pitch, back_thickness, belt_width, max_diameter, angle + rot_angle)
                children(0);
            rotate(rot_angle) translate([0, -radius, 0]) {
                translate([0, -rad_diff / 2, 0]) rotate(-atan(rad_diff / tooth_pitch)) children(0);
                translate([-tooth_pitch / 2, 0, 0]) rotate(-atan(rad_diff / tooth_pitch))
                    linear_extrude(belt_width)
                        polygon([
                            [-rad_diff, 0],
                            [-(rad_diff * back_thickness / 2), -back_thickness],
                            [tooth_pitch + (rad_diff * back_thickness / 2), -back_thickness],
                            [tooth_pitch, 0]
                        ]);
            }
        }
...
module belt_tooth(tooth_profile = undef, belt_width = undef)
{
    ...
    else if (tooth_profile == "belt")    { /* plain belt, no teeth */ }
...
// Note: OpenSCAD module names cannot begin with a digit.
// The dispatcher calls DP40() for the "40DP" profile string.
module DP40(width = 2)
{
